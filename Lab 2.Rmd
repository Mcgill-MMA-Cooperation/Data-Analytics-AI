...
```{r}
model8 <- lm(avg_rating ~ poly(age, degree = 4) + poly(avg_timeplay, degree = 3), data = board_games) 
...
fit=glm(avg_rating~weight, data=board_games)  #note: lm=glm, but glm allows us to do cross-validation
...
for (i in 1:10){
  ...
  mse[i] = cv.glm(board_games,fit,K=5)$delta[1]
}
...
calculate_mse <- function(degrees) {
  ...
  for (fold in 1:K) {
    ...
    folds <- createFolds(1:nrow(board_games), k = K, list = TRUE)
    train_indices <- unlist(folds[-fold])  # Indices for training set
    test_indices <- folds[[fold]]  # Indices for test set
    
    train_data <- board_games[train_indices, ]
    test_data <- board_games[test_indices, ]
